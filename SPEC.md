...
- **The black-hole fault** is specified as one implementable mechanism, a pre-armed finite full-node network partition with a pinned duration, with runtime assertions establishing from observation that it produced both endpoint staleness on the routed path and client-experienced silent drops; it represents node loss for the degradation window only, and its recovery is reported as partition heal, never as node-loss recovery.
- **Time-to-recovery** is decomposed only into boundaries that are actually measurable, with a two-probe split separating replica-readiness from traffic restoration.
- **The load-balancing gate** is conditional on the dataplane's routing regime (§1).
- **The statistics** make no minimum-detectable-effect claim for a single-stack study.
...
Standing caveat: the acceptance criteria certify the instrument against a mock, not any claim about real GPU behaviour. Small N on rented hardware and injected-fault-versus-reality gaps remain; so do mock fidelity limits. They are scoped in the claims and named in the report.
...
**Fault variants:**
- **Clean delete:** grace=0 pod deletion. Endpoint withdrawn near-instantly, kernel RSTs in-flight connections. Labelled "abrupt replica deletion." Never called node loss.
...
- **Receive-path caveat (normative):** TTFT and ITL are timestamped when the user-space read loop observes an SSE content event, not when its bytes reach the host.
  - **Kernel TCP buffering**, server- or proxy-side coalescing of several tokens into one SSE event, and Go runtime scheduling of the reading goroutine add receive-side delay that the client-validity gate cannot detect.
  - **This delay** concentrates in fault and recovery windows, where many stalled streams flush at once.
  - **Two receive-path checks,** both reported per window and neither run-failing in this version:
    - **(i) client-side TTFT** is compared against the server-side TTFT histograms section 2 requires the metrics collector to gather (the scrape lands with the Phase 1 collector, added to the metrics-collector scrape list), and the divergence is reported (the divergence also contains network round-trip time and any definitional difference in where the server starts its TTFT clock, so divergence flags receive-path trouble for investigation);
...
- **Aalen-Johansen completion-incidence curves** (adopted 2026-08-15, CHANGELOG.md) over ALL scheduled requests in the window: the curve is the cumulative incidence of completion, the estimated probability that a scheduled request has completed by time t.
  ...
  - **With zero errors** in a window the estimator reduces exactly to one minus the Kaplan-Meier survival function, so the timeout treatment is unchanged from v0.1.
  - **Report the curve** for baseline, fault, and recovery windows.
  - **A quantile q** is reported only where the curve actually crosses q within the timeout horizon. The crossing test runs in floating point with an absolute tolerance of 1e-9: an incidence at or above q minus 1e-9 counts as a crossing.
    - **Where it does not,** the refusal names which of two cases holds, decided by the ceiling: the final completion incidence plus the event-free survival remaining where the curve ends. Where no terminal event falls after a censoring time, that survival equals the window's censored fraction.
    - **The quantile is unattainable** if and only if that ceiling is below q.
    - **Where the ceiling reaches q,** the refusal reports the quantile as greater than the horizon and prints the ceiling; where the ceiling is below q, it states "unattainable" and prints the final completion incidence and the ceiling.
    - **The greater-than form** carries no reason clause because two cases share it: censored mass may still resolve beyond the horizon, and a re-based completion may cross just past it (§3 send-skew clause).
  - **In a window with errors** the curve may plateau below 1.0 inside the horizon.
  - **The curve is an Aalen-Johansen estimate** of the completion probability under independent right-censoring (right-censoring: observation ends before the outcome arrives).
  - **Censoring is administrative:** it comes from the pinned per-request timeout or from run end on the pre-fixed schedule, never from the request's own behaviour, and the load generator drains, returning only when every scheduled request has reached a terminal state or its timeout, so run-end censoring inside the horizon occurs only on operator abort, and an aborted run is not published.
  - **Under that drain design** the estimate equals the plain completed fraction of the window's scheduled requests at every point inside the horizon; absent the drain, as on an aborted run, the two diverge where censoring patterns vary.
  - **The curve and its crossing times** are descriptive point estimates over the window's scheduled requests, published with n, the count of requests behind each estimate; pointwise interval bands are future work, required before the first comparison table across providers or serving stacks (§11).
...
- **Window (the degraded plateau):** from the fire anchor per §3 plus R (the first R seconds after fire are settling and are excluded) to the earlier of the run's recovery-to-pre-fault entry time or the 600 s fault-window timeout.
  - **R** is the pinned 10 s detector window; the sensitivity sweep varies detector parameters, but the equilibrium baseline itself is always estimated with the pinned R.
- **Estimator:** the equilibrium baseline is the ratio-of-sums goodput over that window, estimable only when the window spans at least R seconds and its goodput is above zero.
  - **A run without an estimable equilibrium** reports "not estimable" with the reason (plateau shorter than R, or zero SLO-goodput during the plateau: no scheduled request met the section 4 SLO, which includes the no-service case); its TTR to equilibrium is N/A, never imputed; the run stays in the published per-run table.
  - **Per-run window bounds,** estimability, and the reason appear in the report JSON (equilibrium_window_start_s, equilibrium_window_end_s, equilibrium_estimable, equilibrium_note).
- **Scope of the term:** a sustained measured operating point within one run.
...
- Pinned and recorded: vLLM version and image digest, model and revision, quantization, max-num-seqs and scheduler settings, chunked-prefill setting, CUDA-graph enablement, GPU SKU, driver, CUDA, cuDNN, NCCL, Kubernetes version, CNI and dataplane mode, kube-proxy mode, layer-7 proxy and version and configuration where deployed (retries disabled, upstream-endpoint response header and client request-ID header named in the pinned configuration), node-monitor-grace-period, client HTTP timeout (30 s), retries (zero), readiness probe configuration, storage medium for weights, and GPU clock and power policy asserted equal across runs (nvidia-smi fingerprint per replica per run).
- Client placement pinned: dedicated non-spot node, same zone and subnet, RTT recorded in the environment table. Client connection floor: 4 per replica, an idle-provisioning minimum added to the demand-driven pool (§1).
...
ignore_eos is a vLLM extension that disables the model's end-of-sequence stop so every completion emits the full max_tokens budget; hosted providers reject or silently ignore it, so hosted requests omit it and completions stop naturally.
...
- **Pre-registered primary endpoint:** TTR to single-replica equilibrium under the clean-delete variant. Everything else is secondary or exploratory and labelled so; Holm correction where several secondaries are formally compared.
...
1. Scaffold and config schema carrying the section 6 pins the Phase 0 profiles use.
...
lambda_max is measured once per Phase 1 environment, before any characterization run, on one replica addressed directly by pod IP (no Service routing), with the experiment's exact pins and load-profile shape (Poisson arrivals, fixed input length, max_tokens=256 with ignore_eos, unique prefixes):

- **Coarse ramp:** start at 2 requests per second and double the rate until a step fails. If the 2 requests-per-second step itself fails, calibration is invalid and no lambda_max is recorded.
- **Fine ramp:** from the last passing rate, increase in steps of 10 percent of that rate until a step fails.

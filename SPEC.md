...
- **Aalen-Johansen completion-incidence curves** (amendment A1) over ALL scheduled requests in the window: the curve is the cumulative incidence of completion, the estimated probability that a scheduled request has completed by time t.
  ...
  - **A quantile q** is reported only where the curve actually crosses q within the timeout horizon.
    - **Where it does not,** the refusal names which of two cases holds, decided by the ceiling: the final completion incidence plus the censored mass outstanding at the horizon.
    - **The quantile is unattainable** if and only if that ceiling is below q.
    - **Where the ceiling reaches q,** the refusal states "greater than" the horizon and prints the ceiling; where the ceiling is below q, it states "unattainable" and prints the final completion incidence and the ceiling.
...
- **Coarse ramp:** start at 2 requests per second and double the rate until a step fails.
...
- **The procedure runs twice**; if the two lambda_max values differ by more than 10 percent, a third ramp decides by median.
